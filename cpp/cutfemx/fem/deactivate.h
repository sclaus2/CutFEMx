...
namespace cutfemx::fem
{
  ...
  template <dolfinx::scalar T, std::floating_point U>
  std::vector<std::int32_t> locate_dofs(std::string deactivate_domain,
                  const std::shared_ptr<dolfinx::fem::Function<T,U>> level_set,
                  const std::shared_ptr<dolfinx::fem::FunctionSpace<U>> V,
                  const std::vector<int>& component)
  {
    ...
    //Communicate values of Function
    xi.x()->scatter_rev(std::plus<U>()); //send data to owner and keep max owner determines max then sends it, // max did not work need to use plus should work anyway
    xi.x()->scatter_fwd(); // send data from owner to ghost having kept the max (sends the max back)
