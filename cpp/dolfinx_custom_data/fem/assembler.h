...
#include <dolfinx/fem/assemble_expression_impl.h>
...
{
  auto [X, Xshape] = e.X();
  dolfinx::fem::impl::tabulate_expression(values, e.kernel(), Xshape,
                                          e.value_size(), coeffs, constants,
                         ...
                         const mesh::Mesh<U>& mesh, fem::MDSpan2 auto entities)
{
  ...
  int cstride = coffsets.back();
  {
    ...
    dolfinx::fem::pack_coefficients(c, coffsets, entities, std::span(coeffs));
  }
  std::vector<T> constants = dolfinx::fem::pack_constants(e);
...
/// @brief Build a sparsity pattern for a runtime CutFEMx form.
/// @note The pattern is not finalised, i.e. the caller is responsible
/// for calling SparsityPattern::assemble.
/// @param[in,out] pattern The sparsity pattern to add to.
...
void build_sparsity_pattern(la::SparsityPattern& pattern, const Form<T, U>& a)
{
  ...
  common::Timer t0("Build CutFEMx runtime sparsity");
...
/// matrix is square.
///
/// CutFEMx deactivation can set diagonal entries on DOFs outside the active
/// runtime integration domain. Those rows may not appear in the cell/entity
/// sparsity generated from the runtime form, so the diagonal has to be reserved
/// explicitly.
template <dolfinx::scalar T, std::floating_point U>
...
/// @brief Create a sparsity pattern for a runtime CutFEMx form.
/// @note The pattern is not finalised, i.e. the caller is responsible
/// for calling SparsityPattern::assemble.
/// @param[in] a A bilinear form.
...
la::SparsityPattern create_sparsity_pattern(const Form<T, U>& a)
{
  ...
  insert_deactivation_diagonal(pattern, a);
  return pattern;
}
